            ...
            <CardContent>
              ...
              <div className="aspect-square bg-gradient-to-br from-primary/10 to-secondary/10 rounded-lg relative overflow-hidden mb-6">
                ...
                <svg className="absolute inset-0 w-full h-full">
                  <path d="M20 380 Q180 300 360 180" stroke="hsl(var(--primary))" strokeWidth="3" strokeDasharray="10,5" fill="none" className="animate-pulse" />
                </svg>

                {/* Driver Location (Moving) */}
                <div className="absolute w-4 h-4 bg-secondary rounded-full animate-pulse" style={{
              ...
              <div className="glass p-4 rounded-lg">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium">Babloo</div>
          ...
          <Card className="glass border-0 shadow-elegant">
            ...
            <CardContent>
              <div className="space-y-6">
                {orderSteps.map((step, index) => <div key={index} className="flex items-start gap-4">
                    {/* Icon */}
                    <div className={`
                      w-10 h-10 rounded-full flex items-center justify-center border-2 smooth
                      ${step.status === 'completed' ? 'bg-primary border-primary text-primary-foreground' : step.status === 'current' ? 'bg-secondary border-secondary text-secondary-foreground animate-pulse' : 'bg-muted border-muted-foreground/30 text-muted-foreground'}
                    `}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h4 className={`font-medium ${step.status === 'current' ? 'text-secondary' : ''}`}>
                          {step.title}
                        </h4>
                        <Badge variant={step.status === 'completed' ? 'default' : step.status === 'current' ? 'secondary' : 'outline'} className="text-xs">
                          {step.time}
                        </Badge>
                      </div>
